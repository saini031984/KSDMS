...
namespace KSDMS
{
    public partial class FRmPassportNew : Form
    {
        int SaveAction;
        public int IntVisaStatus;
        public FRmPassportNew()
        {
            InitializeComponent();
        }

        private void FRmPassportNew_Load(object sender, EventArgs e)
        {
            if (IntVisaStatus == 0) 
            { CmdBack.Visible = false; 
                XLHead.Text = "Passport Reception"; 
                Pnl2.Visible = false; 
            ...
            if (IntVisaStatus == 1)
            {
                ...
                XLHead.Text = "Passport Assessment";
                PnlNewPassport.Visible = false;
            ...
            if (IntVisaStatus == 2)
            {
                ...
                XLHead.Text = "Passport Production";
                BtnAdd.Visible = false;
                PnlCheck.Visible = true;
        ...
        private Boolean Fn_Val()
        {
            ...
            if (CmbStatus.SelectedIndex == 1)
            {
                ...
                {
                    MessageBox.Show("Enter Address Permanent Address (In Nigeria)", GlobalFunction.A_Name, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    TxtNewMotherNm.Focus();
                    BB = false;
                }
                if (TxtNigCIty.Text.Trim() == "")
                {
                    MessageBox.Show("Enter City In Nigeria)", GlobalFunction.A_Name, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    TxtNewMotherNm.Focus();
                    BB = false;
                }
                if (TxtNigCountry.Text.Trim() == "")
                {
                    MessageBox.Show("Enter Country In Nigeria)", GlobalFunction.A_Name, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    TxtNewMotherNm.Focus();
                    BB = false;
                }
        ...
        private void BtnView_Click(object sender, EventArgs e)
        {
            ...
            FrmP.HelpSQL = "Select PassportID as [000,SLNO], FirstNm + ' ' + MidNm + ' ' + LastNm as [200,Name],RegNoStr as [150,RegNo]," +
                ...
                " from PassportApp Where  EntryType='R'  And  PStatus=" + IntVisaStatus + " Order By PassportID";
        ...
        private void BtnRegister_Click(object sender, EventArgs e)
        {
            ...
            {
                ClassPassport DptM = new ClassPassport();
                DptM.Fn_SendFrwd(TxtID.Text, IntVisaStatus, CmbStatus.SelectedIndex);
                Fn_Clear();
            }
            if (IntVisaStatus == 1)
            {
                ...
                if (CmbStatus.SelectedIndex == 0)
                {
                    MessageBox.Show("You are not allowed to register Passport Application, Still Passport status is pending", GlobalFunction.A_Name, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                if (CmbStatus.SelectedIndex == 1)
                {
                ...
                if (CmbStatus.SelectedIndex == 2)
                {
                    ...
                    ClassPassport DMS = new ClassPassport();
                    if (DMS.Fn_PassportRej(TxtID.Text.Trim()) == false)
                    {
                        MessageBox.Show("Please enter why Passport rejected, before Registration.", GlobalFunction.A_Name, MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                    }
                    else
                    {
            ...
            if (IntVisaStatus == 2)
            {
                ...
                if (CmbStatus.SelectedIndex == 2)
                {
                    ...
                    else
                    {
                        ...
                        //PP.Fn_GetNo();
                        //TxtAppNo.Text = PP.StrSrNo;
                        //TxtRefNo.Text = PP.StrSrNo;



                        TxtAppNo.Text = TxtID.Text.Trim();
                        TxtRefNo.Text = TxtID.Text.Trim();
                        TxtEnrolmentNo.Text = TxtID.Text.Trim();
                        TxtRcptAmt.Text = "0";
        ...
        private void CmdBack_Click(object sender, EventArgs e)
        {
            ...
            if (TxtID.Text.Trim() == "") { return; }
            if (MessageBox.Show("Send Back?", GlobalFunction.A_Name, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
            {
                return;
            }
            ClassPassport DptM = new ClassPassport();
            DptM.Fn_SendBack(TxtID.Text, IntVisaStatus, CmbStatus.SelectedIndex);
            Fn_Clear();
        }
        ...
        {
            if (IntVisaStatus == 0) { CmdBack.Visible = false; BtnUpload.Visible = false; return; }
            BtnRegister.Visible = false;
            ClassDMS DMS = new ClassDMS();
            if (TxtID.Text.Trim() == "") { return; }
            if (DMS.Fn_ValFrwdPassport(TxtID.Text.Trim()) == false)
        ...
        private void rectangleShape5_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            CmbNewInitial.SelectedIndex = CmbIntial.SelectedIndex;
            TxtNewLNm.Text = TxtLNm.Text.Trim();
            TxtNewMNm.Text = TxtMNm.Text.Trim();
            TxtNewFNm.Text = TxtFNm.Text.Trim();
            TxtNewPBirth.Text = TxtPBirth.Text.Trim();
            CmbNewGender.SelectedIndex = CmbSex.SelectedIndex;
            CmbNewMartStatus.SelectedIndex = CmbMStatus.SelectedIndex;
            DtNewDOB.Value = DtDOB.Value;
            TxtNewLegGar.Text = TxtLGur.Text.Trim();
            TxtNewMotherNm.Text = TxtMotherNm.Text.Trim();
            TxtNewSpouseNm.Text = TxtSpouseNm.Text.Trim();
        }
