...
namespace KSDMS.DataClass
{
    class ClassLocation
    {
        ...
        private int _CountryNm;
        public int CountryNm
        {
            get
            {
                return _CountryNm;
        ...
        public void Fn_DataView()
        {
            ...
            if (dtC.Rows.Count > 0)
            {
                ...
                _CityNm = dtC.Rows[0]["CityNm"].ToString();
                _CountryNm =Convert.ToInt16(dtC.Rows[0]["CountryNm"].ToString());
        ...
        public void Fn_Save(ref String StrMsg)
        {
            ...
            if (_Action == 1)
            {
                ...
                SQL = "INSERT INTO LocationMaster(LocShort,RegionID,LocType,LocName,LocAddress,LocArea,LocZip,LocPhone,CityNm,CountryNm,IsActive,UpdateBy,LastDate) " +
                    " Values (@LocShort,@RegionID,@LocType,@LocName,@LocAddress,@LocArea,@LocZip,@LocPhone,@CityNm,@CountryNm,@IsActive,@UpdateBy,@LastDate)";
                Com.Connection = Conn;
                Com.Parameters.Clear();
                ...
                Com.Parameters.AddWithValue("@CityNm", _CityNm.Trim().Replace("'", ""));
                Com.Parameters.AddWithValue("@CountryNm", _CountryNm );
            ...
            else if (_Action == 2)
            {
                SQL = "Update LocationMaster SET LocName=@LocName,LocType=@LocType,RegionID=@RegionID,LocAddress=@LocAddress,LocArea=@LocArea,LocZip=@LocZip,LocPhone=@LocPhone," +
                    " IsActive=@IsActive,UpdateBy=@UpdateBy,LastDate=@LastDate, CityNm=@CityNm,CountryNm=@CountryNm " +
